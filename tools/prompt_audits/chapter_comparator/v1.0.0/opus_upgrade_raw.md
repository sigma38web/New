...
The current prompt is a competent pairwise comparator, but it is **genre-blind** in nearly every dimension that matters for Korean webnovel serialization. Specific gaps:

1. **No Cider/Sweet Potato Calibration**: The rubric mentions "serialized_structure" generically but never instructs the judge to evaluate whether a candidate delivers the right frustration-to-catharsis ratio. A chapter that buries its payoff in literary ambiguity would score the same as one that lands a clean cider moment — a fatal flaw for Munpia/KakaoPage readers.

2. **No 절단신공 (Cliffhanger Discipline) Evaluation**: The system mentions "forward pull" in passing but gives no concrete instruction to assess whether the chapter exit cuts at maximum tension/curiosity. The judge needs to explicitly compare how each candidate executes its final beat against the contract's `ending` specification.

3. **No Reaction Economy / 반응 경제 Assessment**: Korean webnovels derive a huge portion of their dopamine from *observer reclassification* — bystanders reacting to the protagonist's reveal, power display, or social upset. The current prompt has zero mechanism to judge whether candidates leverage this.

4. **No Dialogue Register / 티키타카 Evaluation**: The `english_prose_quality` dimension says "natural, idiomatic English" but never addresses the specific challenge of rendering Korean social-register shifts (존댓말 ↔ 반말) into English tonal equivalents, or whether dialogue achieves the fast-paced banter rhythm expected by the genre.

5. **No Dopaminergic Progression Awareness**: The judge has no instruction to check whether a candidate delivers a tangible milestone (stat gain, tier breakthrough, social recognition beat) when the contract calls for one, or whether it defers gratification past the genre's tolerance threshold.

6. **No Anti-Translationese Guardrail in Judging**: The system says "never reward translation-like phrasing" but provides no concrete examples. A judge LLM may not recognize calqued idioms ("courting death," "junior/senior brother," "this daddy") or awkward honorific suffixes left in English prose as penalties without explicit guidance.

7. **No Mobile-Pacing Assessment**: Korean webnovels are overwhelmingly consumed on phones. Paragraph density, white-space rhythm, and 1–4 sentence paragraph convention are structural features, not stylistic preferences. The current prompt only says "never penalize short paragraphs" — it should *actively reward* mobile-optimized pacing when the contract calls for it.

8. **Rubric Dimension List is Implicit**: The system references `english_prose_quality` and `serialized_structure` but the full rubric dimension list is never enumerated in the system prompt. This creates ambiguity about what the judge is actually scoring.

The user template is adequate structurally but should include the series bible / narrative context block and an explicit slot for genre-specific contract metadata (target cider/sweet-potato ratio, required reaction beats, milestone type).
...
### Upgraded user.md
```markdown
[NARRATIVE CONTEXT — series bible excerpt and recent arc state]
{{narrative_context}}
