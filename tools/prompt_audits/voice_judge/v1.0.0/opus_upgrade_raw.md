...
The current prompt is functional but lacks several critical Korean webnovel-specific voice and dialogue evaluation dimensions:

1. **티키타카 (Tikitaka) Banter Assessment Missing**: Korean webnovels rely heavily on rapid-fire conversational ping-pong. The judge has no instruction to evaluate whether dialogue exchanges feel snappy, rhythmic, and responsive versus flat or expository. There is no scoring for banter cadence or comedic timing in dialogue.

2. **Register Dynamics Beyond Violation Detection**: The current prompt treats register as a binary pass/fail against a digest. Korean webnovels use deliberate register friction — a character who suddenly drops from formal to informal speech to signal dominance, intimacy shifts, or emotional breaks. The judge needs to evaluate whether register shifts serve narrative purpose (power dynamics, character development, social reclassification) rather than merely flagging them as violations or non-violations.

3. **Translationese & Anti-Pattern Detection Absent**: There is no instruction to penalize common English-rendered Korean webnovel dialogue anti-patterns: clunky honorific calques ("This hyung will take care of it"), stiff machine-translation idioms ("You dare?", "Courting death"), or unnatural exposition-through-dialogue. The "naturalness of English rendering" instruction is too vague to catch these.

4. **Reaction Economy Voice (반응 경제)**: Korean webnovels frequently use bystander and observer dialogue as a voice dimension — gasps, disbelief murmurs, crowd chatter that reclassifies the protagonist. The judge doesn't evaluate whether observer/reactor voices are distinct from each other or whether they fall into repetitive "What?! How is that possible?!" sameness.

5. **Distinguishability Scoring is Underspecified**: The prompt says "distinguishability" in the family purpose but provides no rubric for it. There's no instruction to check whether two characters could have their name tags swapped without the reader noticing — the core distinguishability test.

6. **Verbal Habit / Idiolect Tracking**: Korean webnovel characters often carry signature verbal tics, catchphrases, sentence-ending patterns, or rhetorical habits. The judge has no instruction to verify these are present when expected and absent when not.

7. **Output Schema Too Thin**: The output shape lacks granularity. There's no per-speaker breakdown, no distinguishability score, no banter quality signal, and no translationese penalty — all of which downstream editorial systems would need.

8. **Cider/Sweet Potato Dialogue Calibration**: The judge doesn't evaluate whether protagonist dialogue delivers satisfying agency and catharsis (cider) versus whether passive, whiny, or indecisive dialogue creates unearned frustration (sweet potato). Voice is a primary vehicle for this.
...
### Upgraded user.md
```markdown
[REGISTER DIGESTS — per-character voice profile including verbal habits, idiolect markers, and current register baseline]
{{register_digests}}

[REGISTER CHECK REPORT — deterministic analysis of register adherence]
{{register_check_report}}

[UTTERANCES — speaker → addressee → text, with paragraph ids]
{{utterances}}

Evaluate all utterances against the Voice Judge framework. For each speaker, assess register fidelity, idiolect consistency, distinguishability, and cider agency. Evaluate multi-turn exchanges for tikitaka rhythm. Flag translationese anti-patterns. Score reactor/bystander voice differentiation if present. Return the single JSON object per the output schema.
```